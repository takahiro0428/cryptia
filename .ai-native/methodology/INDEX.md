...
| バージョン | 日付 | 変更概要 |
|-----------|------|---------|
| 1.12.0 | 2026-07-22 | デプロイパイプライン統合。phase-definitions.md Phase 7 にデプロイパイプラインセクション（自動テストゲート・シークレット管理・手順書・監査検証）を新設し、Phase 4 検討項目の CI/CD 構成を具体化、Phase 7/8 ゲート条件にデプロイパイプライン項目を追加。system-auditor.md AS-3 にデプロイパイプライン監査項目（テストゲート強制・失敗ログ追跡可能性・シークレット管理・手順書整合・排他制御）を追加。phase-gate-checklists.md Phase 7/8 チェックリスト（7-8, 8-11）・リリース判定表を更新。実装リファレンスとして `.github/workflows/deploy.yml`（テストゲート付きデプロイ）、`scripts/setup-deploy-secrets.ps1`（PowerShell シークレット登録）、`guides/deploy-guide.md`（デプロイ手順書）、`guides/ci-cd-setup.md`（CI/CD セットアップ手順書）を新設。ドキュメント整合性検証（`scripts/validate-docs.sh`: バージョン統一・参照実在・シークレット混入）を PR チェックに追加。v1.11.0 で未更新だった core-principles.md / navigator.md のフロントマターバージョン不整合を解消し、全ドキュメントを v1.12.0 に統一 |
