...
## 追加機能（オペレーター要望・同スプリント内）

- **AI 分析のエントリーレンジ表示:** ロング（押し目買いゾーン）/ ショート（戻り売りゾーン）別の
  おすすめ指値レンジを表示。Gemini 提案値は現値 ±50%・min<max で検証し、不正時は
  テクニカル算出（直近サポート/レジスタンス + ボラティリティ σ）へフォールバック
  （`shared/advisor.ts computeEntryRanges` / `server/api/ai/insight.post.ts` / `components/InsightCard.vue`）

## レビュー・監査イテレーション 1 の修正（14 件）

コードレビュアー 11 件（Medium 5 / Low 6）+ システム監査官 3 件（Medium 1 / Low 2、重複 1）を全件修正:
売り確認ダイアログの実行値表示・Token-2022 残高対応・期間別フロー取得管理・
RAG の非対称埋め込み（QUERY/DOCUMENT）・認証済みの IP バックストップ併用・
WS 再接続のソケット同一性ガード・切断時の残高クリア・E303 定数化・
ニュース単語境界照合・ドラッグ中の力加算スキップ・リダイレクト方式の連携フォールバック・
aud/iss の Firebase Auth プロジェクト優先・売り上限計上の保守化（詳細: review-results.md）
