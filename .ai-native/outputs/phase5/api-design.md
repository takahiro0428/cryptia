...
| POST `/api/solana/rpc` | Solana RPC の許可制中継（宛先固定・メソッド別パラメータ検証）。読み取り: `getBalance`/`getTokenAccountsByOwner`。送信系: `sendTransaction`（**署名済み** Tx の中継のみ・base64 検証・サイズ上限）/`getLatestBlockhash`/`getSignatureStatuses`（F-13 ボットウォレット用。プロキシは署名能力を持たない: BR-1 改訂。署名済みであれば第三者の Tx も中継し得るオープンリレー性は IP クォータ + 上流バジェットで有界 — 悪用は正規ユーザーの枠を枯渇させ得る既知の残存リスク） | なし（IP クォータ + 上流バジェット rpc 30/分で保護） |
