...
| Solana `/trade/solana` | 免責、**セッションカード一覧（複数同時実行・個別に一時停止/終了・約定履歴トグル・ポジション行タップでその場詳細: 価格/損益/ラダー残りライン/監査判定/CA・自動スナイプの監視ステータス）**、新規セッションフォーム（割当資金・セッション名・手法タブ: ラダー/新規上場ハンター/自動スナイプ/AI・自動スナイプ設定・戦略ピッカー・上限カウンタ）、新規上場リスト（シグナルチップ + CA 行つきカード）、スコア付きトークンカード（CA 行つき）、過去セッション（タップで約定履歴を展開）、接続障害バナー + 再試行 | トークンカードは 1 列に折返し |
