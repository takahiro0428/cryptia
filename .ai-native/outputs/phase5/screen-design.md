...
| デモトレード `/trade/demo` | 設定フォーム（初期資金・エンジン・戦略表示・銘柄チップ・AIおすすめ）、統計6項目、資産推移チャート、ポジション、注文履歴、アーカイブ | 注文履歴は PC=テーブル / モバイル=カードに切替（原則8） |
| Solana `/trade/solana` | 免責、セッション設定（割当資金・手法タブ・ラダー説明）、実行ダッシュボード、スコア付きトークンカード、警告表示 | トークンカードは 1 列に折返し |
| 実トレード `/trade/live` | 常設免責、ウォレット接続、取引ガード（同意+上限）、注文パネル（AIおすすめ/主要トークンチップ・見積り・AI判断）、確認モーダル、取引履歴（Solscan リンク） | モーダルは 94vw。数値入力は inputmode 指定 |
| 戦略 `/strategy` | 戦略カード一覧（プリセット/カスタム・リスクバッジ・適用ボタン）、作成/編集フォーム（名前・ルール・リスクスライダー） | 2 列 → 1 列に折返し |
