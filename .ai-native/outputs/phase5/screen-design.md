...
| Solana `/trade/solana` | 免責、セッション設定（割当資金・手法タブ: ラダー/新規上場ハンター/自動スナイプ(常時監視)/AI・自動スナイプ設定(最大同時数・監査基準)・戦略ピッカー）、新規上場リスト（シグナルチップ + CA のその場展開・コピー行つきカード）、実行ダッシュボード（価格更新の鮮度表示・ポジション行タップでその場詳細: 価格/損益/ラダー残りライン/監査判定/CA）、スコア付きトークンカード（CA のその場展開・コピー行つき）、過去セッション（タップで約定履歴を展開）、接続障害バナー + 再試行 | トークンカードは 1 列に折返し |
